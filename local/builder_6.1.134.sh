...
# ===== 设置自定义参数 =====
echo "===== 欧加真MT6989通用6.1.134 A16 (天玑特供)OKI内核本地编译脚本 By 王德发刷机 ====="
echo ">>> 读取用户配置..."
...
if [[ "$KSU_BRANCH" == "y" || "$KSU_BRANCH" == "Y" ]]; then
  ...
  VERSION_DEFINITIONS=$'define get_ksu_version_full\nv\\$1-'"$GIT_COMMIT_HASH"$'@王德发刷机\nendef\n\nKSU_VERSION_API := '"$KSU_API_VERSION"$'\nKSU_VERSION_FULL := v'"$KSU_API_VERSION"$'-'"$GIT_COMMIT_HASH"$'@王德发刷机'
  ...
  echo ">>> 最终版本字符串: v${KSU_API_VERSION}-${GIT_COMMIT_HASH}@王德发刷机"
